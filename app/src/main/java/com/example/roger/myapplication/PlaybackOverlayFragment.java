...
public class PlaybackOverlayFragment extends android.support.v17.leanback.app.PlaybackOverlayFragment {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        for (int j = 0; j < movies.size(); j++) {
            ...
            if (mSelectedMovie.getUsername().contentEquals(movies.get(j).getUsername())) {
                mCurrentItem = j;
            }
        }
        */
    ...
    private void updatePlaybackRow(int index) {
        if (mPlaybackControlsRow.getItem() != null) {
            ...
            item.setUsername(mItems.get(mCurrentItem).getUsername());
            //item.setStudio(mItems.get(mCurrentItem).getStudio());
        }
        if (SHOW_IMAGE) {
            updateVideoImage(mItems.get(mCurrentItem).getImageURI().toString());
        }
    ...
    static class DescriptionPresenter extends AbstractDetailsDescriptionPresenter {
        ...
        protected void onBindDescription(ViewHolder viewHolder, Object item) {
            viewHolder.getTitle().setText(((Movie) item).getUsername());
